...
CREATE OR REPLACE FUNCTION arg_index_decrement_trig_func_before_update() RETURNS TRIGGER AS $arg_trig_dec_func$ BEGIN
    UPDATE token SET argumentCounter = argumentCounter - 1, 
    totalCounter= totalCounter- OLD.occurrences WHERE tID = OLD.tID;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS arg_index_dec_trig_before_update ON argument_index;
CREATE TRIGGER arg_index_dec_trig
BEFORE UPDATE 
ON argument_index FOR EACH ROW EXECUTE PROCEDURE arg_index_decrement_trig_func_before_update();
...
CREATE OR REPLACE FUNCTION premise_index_decrement_trig_func_before_update() RETURNS TRIGGER AS $premise_trig_dec_func$ BEGIN
    UPDATE token SET premiseCounter = premiseCounter - 1,
    totalCounter = totalCounter - OLD.occurrences WHERE tID = OLD.tID;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS premise_index_dec_trig_before_update ON premise_index;
CREATE TRIGGER premise_index_dec_trig_before_update
BEFORE UPDATE
ON premise_index 
FOR EACH ROW EXECUTE PROCEDURE premise_index_decrement_trig_func_before_update();
...
CREATE OR REPLACE FUNCTION discussion_index_decrement_trig_func_for_before_update() RETURNS TRIGGER AS $discussion_trig_dec_func$ BEGIN
    UPDATE token SET discussionCounter = discussionCounter - 1,
    totalCounter = totalCounter - OLD.occurrences WHERE tID = OLD.tID;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS discussion_index_delete_trig_before_update ON discussion_index;
CREATE TRIGGER discussion_index_delete_trig_before_update
BEFORE UPDATE 
ON discussion_index 
FOR EACH ROW EXECUTE PROCEDURE discussion_index_decrement_trig_func_for_before_update();